...
int main()
{
    ...
    { 
        ...
        if(arr[i][0][0] == 'H')
        {
            ...
            // strcpy(estab[i][0],arr[i][1]);
            // strcpy(estab[i][1],csaddr);
    ...
    { 
        ...
        else if(arr[i][0][0] == 'T')
        {
            int addr =0;
            // printf("\n%x\n",stringthex(arr[i][1]));
            //For every address in the rec
            for(j=2;j<lines[i];j++)
            {
                printf("%06s changed to %06X\n",arr[i][j],csaddr+stringthex(arr[i][1])+addr);
                addr+=3;
            }
        }
            ...
            for(k=0;k<ct2;k++)
            {
                if(strcmp(estab[k][0],inbuf)==0)
                {
                    if(arr[i][lines[i]-1][0] == '+'){
                        ...
                        printf("M rec (%06X) changed to - %06X moved to %06X\n",oc_val,oc_val+stringthex(estab[k][1]),csaddr+stringthex(arr[i][1]));
                    }
                    else{
                        int oc_val=get_oc(stringthex(arr[i][1]),i);
                        printf("M rec (%06X) changed to - %06X moved to %06X\n",oc_val,oc_val-stringthex(estab[k][1]),csaddr+stringthex(arr[i][1]));
